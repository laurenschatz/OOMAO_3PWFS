classdef utilities
    ...
    methods (Static)
        ...
        function [cent]=pyrcen(image)
            % This function reads in a highly modulated pyramid image,
            %converts it to a binary image, and retrieves the centers of each pupil.
            ave=mean(mean(image));
            im=image>ave;
            BW=mat2gray(im); %convert to binary
            cc=bwconncomp(BW); % detect regions
            stats=regionprops(cc, 'centroid'); % centroid regions
            centroids=cat(1,stats.Centroid);
            cent=round(centroids);
            figure; imagesc(BW) %display centroids
            hold on
            plot(centroids(:,1),centroids(:,2), 'b*'); axis equal
            hold off
        end
        
        
        function videoMaker(image)
            % This function reads in a data cube of dimesions (n,m,z), and turns it into a .avi
            % movie. Each frame is n by m, and it steps through time along
            % dimension z.
            %Uses built in Matlab function VideoWriter
            sz_=size(image);
            prompt='Please enter the name for the name for .avi file  :';
            name=input(prompt,'s');
            filename=strcat(name, '.avi');
            v_=VideoWriter(filename);
            v_.FrameRate=95;
            open(v_)
            figure;
            for i=1:sz_(3)
                imagesc(image(:,:,i)); %caxis([100 500]);
                set(gca,'nextplot','replacechildren')
                frame_=getframe(gcf);
                writeVideo(v_,frame_)
            end
            close(v_)
            
        end
        
        function [circ] = circle(s,nPx,varargin)
            
            % c = circle(s, nPx)
            % c = circle(s, nPx, [x y])
            % Create an array of zeros of size s by v with a circle of diameter nPx filled with ones
        ...
        function [strehl,radotf,perfradOtf, r]= psf2strehl(psf, perfPsf)
            
            otf=(abs(fftshift(psf2otf(psf))));
            sz=size(otf);
            perfOtf=(abs(fftshift(psf2otf(perfPsf))));
            
            %% Radial Average
            %Data
            [radotf,r]=radialavg(otf,sz(1)/2+1,0,0);
            radotf=radotf(2:end); %chop off NAN value
            r=r(2:end); %chop off NAN value
            radotf2=radotf(3:end); %chop off noise term
            r2=r(3:end);
            f=polyfit(r2,radotf2,8);%fit
            y1=polyval(f,r);
            radotf(1:3)=y1(1:3);%replace data with fit
            radotf=radotf(1:78);
            %perfect
            [perfradOtf,r]=radialavg(abs(perfOtf),sz(1)/2+1,0,0);
            perfradOtf=perfradOtf(2:79); % chop off NAN value and everything past cuttoff freq
            r=r(2:79); % chop off NAN value
            
            %% Normalize
            Max=max(max(radotf));
            radotf=radotf./Max;
            
            perfMax=max(max(perfradOtf));
            perfradOtf=perfradOtf./perfMax;
            
            %%Strehl
            strehl=sum(radotf)/sum(perfradOtf);
            
        end
        
        
                function [strehl,radotf,perfradOtf, r]= experimentalpsf2strehl(psf, perfradOtf)
        ...
        function [imageCube,nImage] = readMPTiff(filename)
            ...
            function [strehl,otf,perfOtf]= simpsf2strehl(psf, perfPsf)
                             
            otf=((fftshift(psf2otf(psf))));
            sz=size(otf);
            perfOtf=((fftshift(psf2otf(perfPsf))));
            
            %% Mask off anything past the radius of correction
            mask=utilities.circle(sz(1),18);
            otf=otf.*mask;
            perfOtf=perfOtf.*mask;
...
%             
                %chop off NAN value
%                         %%
%             radotf2=radotf(2:end); %chop off noise term
%             r2=r(2:end);
%             f=polyfit(r2(1:2),radotf2(1:2),1);%fit
%             y1=polyval(f,r);
%             radotf(1)=y1(1);%replace data with fit
%             radotf=radotf(1:26);
        
%             %% perfect
%             [perfradOtf,r]=radialavg(abs(perfOtf),sz(1)/2+1,0,0);
%             perfradOtf=perfradOtf(2:end); % chop off NAN value and everything past cuttoff freq
%             r=r(2:end); % chop off NAN value
%             

% radotf=otf(:,41);
% perfradOtf=perfOtf(:,41);

            %% Normalize
            Max=max(max(otf));
            otf=otf./Max;
            %%
            perfMax=max(max(perfOtf));
            perfOtf=perfOtf./perfMax;
            
            %% Strehl
            strehl=abs(sum(sum(otf))/sum(sum(perfOtf)));
