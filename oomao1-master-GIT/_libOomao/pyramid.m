classdef pyramid < handle
    ...
    properties
        altSlopes;                  %Use alternative form of 3PWFS
        nLenslet                    % # of lenslet
        ...
        nFaces;                     %Type of pyramid WFS. Enter a number for the number of faces
        rotation;                   %rotation of the PWFS mask
        alternative;                % alternative 3pwfs mask that gives it tilts like a 4pwfs.
        
        
        
    end
    properties(GetAccess = 'public', SetAccess = 'private')
        centroids                   %Location of Pupil Centers;
    ...
    methods
        ...
        function pwfs = pyramid(nLenslet,resolution,varargin)
            p = inputParser;
            addParameter(p, 'altSlopes',0,@isnumeric);
            addParameter(p,'alternative', 0,@isnumeric);
            addRequired(p,'nLenslet',@isnumeric);
            addRequired(p,'resolution',@isnumeric);
            addParameter(p,'nFaces', 4, @isnumeric);
            ...
            addParameter(p,'modulationCalibPwfs',20,@isnumeric);
            ...
            pwfs.altSlopes              = p.Results.altSlopes;
            pwfs.alternative            = p.Results.alternative;
            ...
            pwfs.nFaces                 = p.Results.nFaces;
            pwfs.rotation               = p.Results.rotation;            
            ...
            if pwfs.altSlopes==2
                pwfs.validSlopes= [pwfs.validDetectorPixels];
            %pwfs.validSlopes            = [flip(flip(pwfs.validDetectorPixels,1),2) flip(flip(pwfs.validDetectorPixels,1),2)];
            end
            ...
            if pwfs.altSlopes==1
                pwfs.referenceSlopesMap = zeros(pwfs.nLenslet*pwfs.c/pwfs.binning,pwfs.nLenslet*3*pwfs.c/pwfs.binning);
            end
             if pwfs.altSlopes==2
             pwfs.referenceSlopesMap = zeros(2*pwfs.nLenslet*pwfs.c/pwfs.binning);
            end
        ...
        function setvalidDetectorPixels(pwfs,val)
            ...
            if nargin == 2 % user custom value
                ...
                if pwfs.altSlopes==1
                pwfs.validSlopes = [pwfs.validDetectorPixels pwfs.validDetectorPixels pwfs.validDetectorPixels];    
                end
                 if pwfs.altSlopes==2
                pwfs.validSlopes = [pwfs.validDetectorPixels];    
                end
            end
            if nargin == 1 && pwfs.tel.D < 0
                ...
                pwfs.validDetectorPixels = logical(pwfs.validDetectorPixels - centralHole);        
                  ...
                  if ~pwfs.isInternalCalibrationPWFS && pwfs.tel.D > 0  % if a telescope and a source are defined a criterion with the
                % minLightRation is applied
                pwfs_ = pyramid(pwfs.nLenslet,pwfs.resolution,'nFaces',pwfs.nFaces,'alternative', pwfs.alternative,'src',pwfs.src, 'altSlopes', pwfs.altSlopes, 'alpha',pwfs.alpha,'rotation',pwfs.rotation,'modulation',10,'alpha',pwfs.alpha,'c',pwfs.c,...
                ...
                %phase=pwfs.src.phase;
                %theCommonPathError={pwfs.tel.pupil,phase};
                pwfs.src = pwfs.src.*pwfs.tel* pwfs_; % propagate through
                %pwfs.src.phase=phase;
                ...
                img   = pwfs_.camera.frame;
                img  = utilities.binning(img,size(pwfs_.camera.frame)/pwfs_.binning);
                %img = padarray(img, [pwfs_.nLenslet,pwfs_.nLenslet],0,'both');
                [centroids]=utilities.pyrcen(img); % Order of pupils for the centroids is (for 4PWFS): center(1,:) top left, center(2,:) bottom left, center(3,:) top right, center(4,:) bottom right               
                sz=size(centroids);
                szFF=size(img)

           
                sz2=size(zeros(pwfs.nLenslet*pwfs.c/pwfs.binning,pwfs.nLenslet*2*pwfs.c/pwfs.binning));
                %%
                for i=1:sz(1)
                    frame{i}=img(centroids(i,2)-sz2/2:centroids(i,2)+sz2/2-1,centroids(i,1)-sz2/2:centroids(i,1)+sz2/2-1);
                end
                %%
                sz3=size(frame{1});
                totalIntensity4Q=zeros(sz3(1),sz3(2));
                for i=1:sz(1)
                    totalIntensity4Q = totalIntensity4Q+frame{i};
                end
                %%
                if pwfs_.graphicalDisplay == 1
                    subplot(1,3,2)
                ...
                pwfs.validDetectorPixels = utilities.piston((pwfs.nLenslet)/pwfs.p_binning,...
                    pwfs.nLenslet*pwfs.c/pwfs.p_binning,'type','logical','xOffset',0,'yOffset',0);
                if pwfs.altSlopes==2
                    Mask=zeros(szFF(1),szFF(2));
                  for i =1:sz(1)
                      Mask=Mask+utilities.circle(szFF(1),pwfs.nLenslet+1,[centroids(i,1), centroids(i,2)]);
                  end
                pwfs.validDetectorPixels=Mask;            
                pwfs.validSlopes = [pwfs.validDetectorPixels];    
                end
                %pwfs.validDetectorPixels = totalIntensity4Q>pwfs.minLightRatio*max(totalIntensity4Q(:));
                pwfs.centroids=centroids;
...
%             end
        ...
        function setValidLenslet(pwfs,val,nL)
            ...
            if nargin == 3 %select only the nLenslet central pixels
                ...
                if pwfs.altSlopes==2
                    pwfs.validLenslet=pwfs.validDetectorPixels;
                end
                 end 
            ...
            if pwfs.altSlopes==1
            pwfs.validSlopes = [pwfs.validLenslet pwfs.validLenslet pwfs.validLenslet];    
            end
            if pwfs.altSlopes==2
             
            pwfs.validLenslet=ones(pwfs.nLenslet*pwfs.c.*2/pwfs.binning, pwfs.nLenslet*pwfs.c.*2/pwfs.binning);
            pwfs.validSlopes = [pwfs.validLenslet];    
            end
        end
        %% Get and Set alpha
        ...
        function set.binning(pwfs,val)
            ...
            if pwfs.altSlopes==1
                pwfs.referenceSlopesMap = zeros(pwfs.nLenslet*pwfs.c/pwfs.p_binning,pwfs.nLenslet*3*pwfs.c/pwfs.p_binning);
            end
            if pwfs.altSlopes==2
                sz=size(pwfs.validDetectorPixels);
                pwfs.referenceSlopesMap=zeros(sz(1),sz(2));   
            end
        ...
        function varargout = slopesDisplay(pwfs,varargin)
            ...
            if pwfs.altSlopes==1
            pwfs.validSlopes=[flip(flip(pwfs.validDetectorPixels,1),2) flip(flip(pwfs.validDetectorPixels,1),2) flip(flip(pwfs.validDetectorPixels,1),2)];
            data = reshape( pwfs.slopesMap , n , 3*n, []);    
            end
            if pwfs.altSlopes==2
            pwfs.validSlopes=[flip(flip(pwfs.validDetectorPixels,1),2)]  
            data = reshape( pwfs.slopesMap , n , n, []);
...
%            if ~pwfs.isInternalCalibrationPWFS && pwfs.tel.D > 0 
            ...
            I4Q = utilities.binning(frame,size(pwfs.camera.frame)/pwfs.binning);             % binning
            I4Q = reshape(I4Q,n,n,[]);
            %I4Q = padarray(I4Q, [pwfs.nLenslet,pwfs.nLenslet],0,'both');  
            
            %n = size(I4Q,1);
            nFrames = size(I4Q,3);
            sz=size(pwfs.centroids);
            sz2=size(zeros(pwfs.nLenslet*pwfs.c/pwfs.binning,pwfs.nLenslet*2*pwfs.c/pwfs.binning));
if pwfs.altSlopes ~= 2
               if nFrames>1
                   for i=1:sz(1)
                       im{i}=I4Q(pwfs.centroids(i,2)-sz2/2:pwfs.centroids(i,2)+sz2/2-1,pwfs.centroids(i,1)-sz2/2:pwfs.centroids(i,1)+sz2/2-1,:);
                   end
                   %%
               else
                   for i=1:sz(1)
...
%           
%             else
%             fprintf('Im doing something else');    
%             n = size(pwfs.camera.frame,1)/pwfs.binning;
%             frame = pwfs.camera.frame;
%             frame = (frame - pwfs.flatField)./pwfs.pixelGains;
%             
...
%             
%             end
             if ~pwfs.isInternalCalibrationPWFS %only compute slopes if it is not a calibration 
            computeSlopes(pwfs, im);% modif
             end
        end
        ...
        function pwfs = pyramidTransform(pwfs,wave)
            ...
            if pwfs.modulation>0
                    ...
                    if gpuDeviceCount == 0 %|| numel(pwfs.q) > 150000000 % if the required memory is too large to use a gpu...
                        ...
                        for kTheta = 1:pwfs.nTheta
                            ...
                            if pwfs.viewFocalPlane
                                ...
                                figure(1112); imagesc(pwfs.fpIm); axis equal; axis tight;
                                drawnow
                            end
                ...
                if pwfs.viewFocalPlane
                    ...
                    figure(1111); imagesc(pwfs.fpIm); axis equal; axis tight;
                    drawnow
                end
                I4Q = abs(fft2(I4Q)).^2;
            end
