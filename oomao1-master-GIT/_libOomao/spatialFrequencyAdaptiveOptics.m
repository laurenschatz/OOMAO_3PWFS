classdef spatialFrequencyAdaptiveOptics < handle
    ...
    methods
        ...
        function obj = spatialFrequencyAdaptiveOptics(tel, atm, ...
                nActuator, noiseVariance, loopGain, samplingTime, latency, resolution, filter, MV, modulation, varargin)
            expectedFilters = {'Rigaut','Fried','Hudgin','Anti-Alias','Southwell','hudgin', 'fried', 'southwell', 'rigaut', 'antialias', 'freischlad86','pyr','pyramid'};
            ...
            addOptional(inputs,'nTimes',3,@isnumeric);
        ...
        function [out, out1] = aliasingPSD(obj,fx,fy)
            if nargin < 2
                fx = obj.fx;
                fy = obj.fy;
            end
            ...
            [PSD_ALIASING] = mk_propagated_aliased_PSD(fx, obj.nTimes, d, r0, obj.atm.L0, localRx, localRy, vx, vy, Ts, td, obj.tel.D, tf, obj.filter);
        ...
        function varargout = image(obj,resolution,pixelScaleInMas, telOtf)
            ...
            psd = powerSpectrumDensity(obj,fx_,fy_)*(wvlRatio)^2;
            ...
            imagesc(alpha,alpha,log10(obj.psf.^(1/obj.psfRoot)))
    ...
    methods (Static)
        ...
        function fao = psdPaper(atmCase,aoSystem,mvFlag, filterType,CTYPE,mag)
            ...
            % SH WFS NOISE :: (values computed in caseStudy.m for SPHERE case, RON=2, CoG centroiding)
            ...
            switch aoSystem
                case 'general'
                    ...
                    nTh_ = 1;
                    fao = spatialFrequencyAdaptiveOptics(d_tel,d_atm,41,noiseVar,CTYPE*0.5,T,tau,resolution,filterType,mvFlag,nTh_);
                case 'sphere'
                    %--------------------------------------------------------------
                    % VLT-SPHERE-like
                    d_tel = telescope(8);
                    ...
                    %noiseVar = 1;
                    if strcmp(filterType,'pyr')
                        noiseVar = noiseVar/pi^2;
                    end
                    fao = spatialFrequencyAdaptiveOptics(d_tel,d_atm,41,noiseVar,CTYPE*0.5,2e-3,2.8e-3,resolution,filterType,mvFlag);
                case 'keck'
                    %--------------------------------------------------------------
                    % Keck-like
                    ...
                    nTh_ = 1;
                    fao = spatialFrequencyAdaptiveOptics(d_tel,d_atm,20,noiseVar,CTYPE*0.5,Ts,tau,resolution,filterType,mvFlag,nTh_);
                ...
                case 'hScaoElt'
                    ...
                    nTh_ = 1;
                    fao = spatialFrequencyAdaptiveOptics(d_tel,d_atm,74,noiseVar,CTYPE*0.5,Ts,tau,resolution,filterType,mvFlag,nTh_);
            end
            ...
            legend('noise','aliasing','aniso-servo-lag','high-frequency','fontsize',16)
            ...
            o = linspace(-1,1,fao.psfResolution-1)*pixelScaleInMas*(1.65/0.5)/1000*fao.psfResolution/2;
