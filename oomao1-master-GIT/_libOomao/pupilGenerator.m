    ...
    methods
        ...
        function mkSpiders(obj,spiderStruct)
            ...
            if (nSpiders~=0) && ~spiderUsrDefined
                %                 %% If Image Processing Toolbox not available
                %                 phi=atan(spidersWidth/(2*obj.D));
                %                 theta = [-phi,phi,-phi+pi,+phi+pi];
                %                 [X,Y]=meshgrid(linspace(-1,1,ceil(sz/2)), linspace(-1,1,ceil(sz/2)));
                %                 spiders=zeros(ceil(sz/2));
                %
                %                 for k=1:nSpiders
                ...
                %                 end
                %                 spiders=logical(psfrTools.interpolate(double(spiders),sz));
        ...
        function out=zeroPad(obj,ratio,flagReplace)
            ...
            tic
            out=psfrTools.enlargePupil(obj.matrix,ratio);
            if flagReplace
                obj.matrix=out;
        ...
        function out=resize(obj,newNPixels,flagReplace,method)
            ...
            if flagReplace
                obj.matrix=out;
                %obj.segRef.matrix=psfrTools.interpolate(obj.segRef.matrix,round(obj.segRef.nPixels*newNPixels/obj.nPixels));
